...
class PercentBollingerBands(Indicator):
    ...
    def compute(self, data:pl.Series, span:int=20) -> pl.Series:
        try:
            ...
            self._set_values((data - lower_band) / (upper_band - lower_band))
            self.logger.info("Computed Percent Bollinger Bands successfully.")
        except Exception:
            self.logger.exception(f"Error in compute:")
            raise
...
class ATR(Indicator):
    ...
    def compute(self, high: pl.Series, low: pl.Series, close: pl.Series, window: int = 14) -> pl.Series:
        try:
            ...
            tr = tr_df.select(pl.max_horizontal(["tr1", "tr2", "tr3"]))["tr1"]
...
class WilliamsR(Indicator):
    ...
    def compute(self, high: pl.Series, low: pl.Series, close: pl.Series, window: int = 14) -> pl.Series:
        try:
            ...
            lowest_low = low.rolling_max(window_size=window)
...
class CMF(Indicator):
    ...
    def compute(self, high: pl.Series, low: pl.Series, close: pl.Series, volume: pl.Series, window: int = 20) -> pl.Series:
        try:
            ...
            cmf = mf_volume.rolling_sum(window_size=window) / volume.rolling_sum(window_size=window)
